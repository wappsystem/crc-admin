...
var _set_headers=function(){
    ...
    if(_table!=={}){
        ...
        _table.Modified={readOnly:true};
        _table.MAuthor={readOnly:true};
        for(var i=0;i<ay.length;i++){
...
var _set_req=function(){
    var sql="with tb as (select Information,ID,UID,PUID,DateTime,Author,Modified,MAuthor,RowNum=row_number() over (order by ID DESC) from [TABLE-"+_db_pid+"-@S1] )";
...
var _set_req_export=function(){
    var sql="with tb as (select Information,ID,UID,DateTime,Author,Modified,MAuthor,RowNum=row_number() over (order by ID DESC) from [TABLE-"+_db_pid+"] )";
...
var _render=function(I){
    ...
    for(var i=start;i<max;i++){
        ...
        if(_records[i].Modified!==undefined){
    ...
    var txt="";
    txt+="<tr><th><u id=refresh__ID style='cursor:pointer'><b>↻</b></u></th>"
    //-------------------------
    _create_header();
    //-------------------------
    ...
    $('#grid__ID td').each(function(){
        ...
        if(column_name=='_Form'){
            ...
            $(this).css('color','#666')
            $(this).html("<u style='cursor:pointer'>Form</u>");
        ...
        if(column_name=='_Delete'){
            $(this).css('color','#666')
            $(this).html("<u style='cursor:pointer'>Delete</u>");
        ...
        //-------------------------
        if(column_name=='_Form' || column_name=='_Delete' || column_name=='DateTime' || column_name=='Author' || column_name=='Modified' || column_name=='MAuthor' || _records[row].vm_readonly[column_name]===true){
...
var _row_data=function(I){
    ...
    for(var i=0;i<_headerB.length;i++){
        var a=_headerA[i][0];
        var b=_headerB[i];
        if(_headerA[i]=='_Hidden' || _headerA[i]=='_gridHidden' || (a!='_' && b!=="ID" && b!=="DateTime" && b!=="Author" && b!=="Modified" && b!=="MAuthor") ){
...
$('#new__ID').on('click', function(){
    ...
    for(var i=0;i<_headerB.length;i++){
        var b=_headerB[i];
        if(b!=="ID" && b!=="DateTime" && b!=="Author" && b!=="Modified" && b!=="MAuthor" && b!=="_Form" && b!=="_Delete"){
            new_row[b]="";
        }
    }
